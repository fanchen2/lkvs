...
def _check_host_split_lock_cmdline():
    ...
    try:
        ...
        host_cmdline = result.stdout
        if isinstance(host_cmdline, bytes):
            host_cmdline = host_cmdline.decode().strip()
        else:
            host_cmdline = host_cmdline.strip()
...
def _enable_kvm_trace():
    ...
    try:
        process.run("echo 1 > /sys/kernel/debug/tracing/events/kvm/kvm_exit/enable",
                   shell=True)
...
def _check_exception_nmi_in_trace(test, expect_present, retry_count=0):
    ...
    if retry_count > 0:
        ...
        for i in range(retry_count):
            try:
                output = process.run("cat /sys/kernel/debug/tracing/trace | "
