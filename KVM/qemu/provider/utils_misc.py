...
def dmesg_time(session=None):
    ...
    if session:
        status = session.cmd_status(kmsg_cmd)
        output = session.cmd_output(show_time_cmd)
    else:
        status = process.system(kmsg_cmd, shell=True)
        output = process.system_output(show_time_cmd, shell=True).decode()
    if status:
        raise exceptions.TestError("Failed to access /dev/kmsg, or failed to get dmesg timestamp, please check your Kconfig and dmesg")
    raw_timestamp = re.search(r"\[(\w+ \w+ \d+ \d{2}:\d{2}:\d{2} \d{4})\]", output).group(1)
